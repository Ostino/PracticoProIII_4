...
public class Test {
    ...
    private final String Operacion_llenado =
            "^\\s*(rellenar)\\s+([0-9]{1,3}),([0-9]{1,3})\\s*$";
    ...
    public String procesarSalida() throws InterruptedException {
        ...
        if (fillflag) {
            modelo.setBaldePintura(true);
            Pattern patronExpRegFill = Pattern.compile(Operacion_llenado);
            Matcher Fill = patronExpRegFill.matcher(comandoEntrada);
            Fill.find();
            int x = Integer.parseInt(Fill.group(2));
            int y = Integer.parseInt(Fill.group(3));
            System.out.println("Entre a pintar");
            modelo.Airededor(x,y);
            modelo.setBaldePintura(false);
            fillflag= false;
        }
        if (rgbflag) {
    ...
    private boolean validarEntrada() {
        if (comandoEntrada.matches(Operacion_llenado)){
